...
risk_mod = model_data_long %>%
  ...
  mutate(
    ...
    fit_modNM = map(likdat,  ~optim(c(-12, -30), surv_logLik, likdat = .x)),
...
exposure_heat = risk_grid %>%
  ...
  geom_text(fontface = "bold", colour = "white") +
...
risk_heat = crossing(
  ...
  scale_x_continuous(expression(paste("Secondary child log"[10], " VL (DNA copies/swab)")),
                     breaks = 0:6+0.5,
                     labels = paste(0:6, 1:7, sep = "-")) +
  # scale_fill_gradient2("Weekly infection prob.",
  #                      low = "#fee0d2",
  #                      mid = "#fc9272", midpoint = 0.05,
  #                      high = "#de2d26",
  #                      breaks= c(0:5/20),
  #                      labels = c(0:4/20, " >0.25")) +
  scale_fill_distiller("Weekly infection prob.", palette = "RdYlBu") +
...
Make long datasets with infected only and removal of x% interpolated households
...
risk_mod %>%
 ggplot(aes(x = idpar, y = betaE, colour = cohort, shape = pvalue < 0.05)) +
  ...
  scale_y_log10("Estimated weekly exposure risk coefficient", limits = c(10^-15, 1)) +
